...
run_enrichment <- function(gene_list, databases, readable = FALSE, verbose = FALSE) {
  ...
  if ("Reactome" %in% databases) {
    ...
    enrich_results$Reactome <- quiet(as.data.frame(
  ...
  if ("GO" %in% databases) {
    ...
    enrich_results$GO <- quiet(as.data.frame(
  ...
  if ("KEGG" %in% databases) {
    ...
    kegg_enrich <- quiet(enrichKEGG(gene = gene_list, organism = "hsa", pvalueCutoff = 1, qvalueCutoff = 1, pAdjustMethod = "BH"))
    if (readable) {
