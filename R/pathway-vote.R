...
run_enrichment <- function(gene_list, databases, readable = FALSE, verbose = FALSE) {
  ...
  if ("Reactome" %in% databases) {
    ...
    enrich_results$Reactome <- suppressMessages(as.data.frame(
  ...
  if ("GO" %in% databases) {
    ...
    enrich_results$GO <- suppressMessages(as.data.frame(
  ...
  if ("KEGG" %in% databases) {
    ...
    kegg_enrich <- suppressMessages(enrichKEGG(gene = gene_list, organism = "hsa", pvalueCutoff = 1, qvalueCutoff = 1, pAdjustMethod = "BH"))
    if (readable) {
